...
## Sección 09 — Arquitectura de referencia
...
El diagrama de la Sección 07 representaba el ciclo de gobierno de PromptOps como proceso. Este diagrama extiende ese ciclo incorporando la Aplicación y el LLM como componentes del sistema que reciben y ejecutan los prompts desplegados, cerrando el circuito entre la operación y la mejora continua.
...
| Despliegue | Distribuir la nueva versión de manera controlada. |
| Aplicación | Consumir el prompt y construir la solicitud al modelo. |
| LLM | Generar la respuesta a partir del prompt desplegado. |
...
El recorrido de este capítulo transformó la perspectiva sobre el prompt: de instrucción aislada a componente de una arquitectura operativa. Esa perspectiva es el punto de partida del próximo capítulo, donde el diseño de interacciones sostenidas en el tiempo exige un nivel adicional de control sobre el contexto, la memoria y el estado de la sesión.
