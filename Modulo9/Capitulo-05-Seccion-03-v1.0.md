...
- Confused deputy problem aplicado a agentes: el agente actúa en nombre del usuario (con sus credenciales, permisos y contexto) pero puede ser manipulado por el atacante (via prompt injection) para usar esas credenciales para acciones que el usuario no autorizó — el agente es el "deputy" confundido entre dos principals con intenciones opuestas
