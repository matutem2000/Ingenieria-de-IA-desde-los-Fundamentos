...
- Serving plane: model server (Triton/KServe) con autoscaling via KEDA, API Gateway con rate limiting y autenticación JWT, load balancer con routing por canary weight o header-based routing
- Control plane: model registry con webhooks de promoción, pipeline orchestrator con DAGs declarativos, configuration management via GitOps (ArgoCD), y secret management (HashiCorp Vault)
