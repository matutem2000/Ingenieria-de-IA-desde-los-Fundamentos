...
- Composición secuencial tipada: cada función de un pipeline devuelve un tipo Pydantic específico que es la entrada de la siguiente; la cadena `extraction_result = extract(text); validated = validate(extraction_result); final = generate(validated)` detecta type errors en tiempo de desarrollo via mypy
