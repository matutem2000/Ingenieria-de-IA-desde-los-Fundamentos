...
- Langfuse tracing via callback: `from langfuse.callback import CallbackHandler; handler = CallbackHandler(); chain.invoke(input, config={"callbacks": [handler]})` captura toda la cadena LangChain como traza jerárquica en Langfuse sin modificar el código de la cadena
