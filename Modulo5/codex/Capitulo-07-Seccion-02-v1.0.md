...
- RAGAS `evaluate()`: recibe un `Dataset` de HuggingFace con columnas `question`, `answer`, `contexts`, `ground_truth` y devuelve un dict con el score medio de cada métrica; integración directa con LangChain y LlamaIndex vía `from ragas.integrations.langchain import evaluate`
