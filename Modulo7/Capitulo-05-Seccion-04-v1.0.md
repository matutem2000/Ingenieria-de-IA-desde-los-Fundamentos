...
- **RunContext y dependency injection**: Pydantic AI usa un sistema de inyección de dependencias (`RunContext[Dependencies]`) que provee a cada herramienta y al agente acceso a servicios externos (base de datos, HTTP client, configuración) de forma type-safe sin variables globales
...
- **Streaming tipado**: el método `agent.run_stream()` devuelve un stream donde los eventos intermedios (thoughts, tool calls) son objetos tipados, no texto libre, facilitando la construcción de UIs reactivas que muestran el progreso del agente

## Principio rector

Pydantic AI aplica el principio de "fail fast" a la integración agéntica: en lugar de descubrir en producción que la salida del agente no tiene el formato esperado, la validación en tiempo de ejecución garantiza que el output sea correcto antes de que llegue al código consumidor.
